...
import { sendNewPIN, sendOtpForgetPIN } from "../../../../services/merchantServices";
...
const OtpRequestForgetPIN = ({ navigation, route }) => {
  ...
  const handleVerifyOTP = async () => {
    try {
      console.log("Mengirim verifikasi PIN baru...");

      const { email } = payload;

      await sendNewPIN(otp, email);

      console.log("Verifikasi PIN baru berhasil.");

      setPopUp(true);
      setTimeout(() => {
        navigation.navigate("success-new-PIN", { email });
      }, 3000);
    } catch (error) {
